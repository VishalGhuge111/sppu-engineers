...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const competitionTypeSelect = document.getElementById("competition-type") // Get reference
    const competitionFeeContainer = document.getElementById("competition-fee-container") // Get reference
    const competitionFeeInput = document.getElementById("competition-fee") // Get reference
    ...
    if (competitionsForm) {
        ...
        // --- NEW CODE FOR TYPE CHANGE LISTENER ---
        if (competitionTypeSelect && competitionFeeContainer && competitionFeeInput) {
            competitionTypeSelect.addEventListener("change", () => {
                if (competitionTypeSelect.value === "paid") {
                    competitionFeeContainer.classList.remove("hidden")
                    competitionFeeInput.required = true
                    competitionFeeInput.focus() // <--- ADD THIS LINE
                } else {
                    competitionFeeContainer.classList.add("hidden")
                    competitionFeeInput.required = false
                    competitionFeeInput.value = "" // Clear the value when hidden
                }
            })

            // Also, call it once on load to ensure initial state is correct
            // (e.g., if the form loads with "paid" pre-selected during an edit)
            if (competitionTypeSelect.value === "paid") {
                competitionFeeContainer.classList.remove("hidden")
                competitionFeeInput.required = true
            } else {
    ...
    onValue(
        ...
        (snapshot) => {
            ...
            if (competition) {
                ...
                const competitionFeeInput = document.getElementById("competition-fee") // Get reference here as well

                if (competition.type === "paid") {
                    feeContainer.classList.remove("hidden")
                    competitionFeeInput.required = true
                    competitionFeeInput.focus() // <--- ADD THIS LINE when editing
                } else {

...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Get references to the specific elements for resources
    const resourceTypeSelect = document.getElementById("resource-type")
    const resourcePriceContainer = document.getElementById("resource-price-container")
    const resourcePriceInput = document.getElementById("resource-price")
    ...
    if (resourcesForm) {
        ...
        // --- NEW CODE FOR TYPE CHANGE LISTENER (Resources) ---
        if (resourceTypeSelect && resourcePriceContainer && resourcePriceInput) {
            resourceTypeSelect.addEventListener("change", () => {
                if (resourceTypeSelect.value === "paid") {
                    resourcePriceContainer.classList.remove("hidden")
                    resourcePriceInput.required = true
                    resourcePriceInput.focus() // <--- ADD THIS LINE: Focus the input
                } else {
                    resourcePriceContainer.classList.add("hidden")
                    resourcePriceInput.required = false
                    resourcePriceInput.value = "" // Clear the value when hidden
                }
            })

            // Also, call it once on load to ensure initial state is correct
            // This handles cases where a form might load with 'paid' pre-selected (e.g., during an edit)
            if (resourceTypeSelect.value === "paid") {
                resourcePriceContainer.classList.remove("hidden")
                resourcePriceInput.required = true
            } else {
    ...
    onValue(
        ...
        (snapshot) => {
            ...
            if (resource) {
                ...
                const resourcePriceInput = document.getElementById("resource-price") // Get reference here as well
                if (resource.type === "paid") {
                    priceContainer.classList.remove("hidden")
                    resourcePriceInput.required = true
                    resourcePriceInput.focus() // <--- ADD THIS LINE: Focus the input when editing
                } else {
